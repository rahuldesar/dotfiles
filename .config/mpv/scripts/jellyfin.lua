...
local function update_overlay()
	...
	local base_url = options.url
		...
		.. "&parentId="
		.. parent_id[layer]
		.. "&enableImageTypes=Primary&imageTypeLimit=1&fields=PrimaryImageAspectRatio,Taglines,Overview"
	if layer == 2 then
...
local function play_video()
	...
	mp.commandv(
		"loadfile",
		options.url .. "/Videos/" .. items[selection[layer]].Id .. "/stream?static=true",
		"insert-at-play",
		selection[layer] - 1
	)
